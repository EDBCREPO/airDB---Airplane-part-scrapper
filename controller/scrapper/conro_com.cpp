...
namespace controller { namespace scrapper {
    ...
    promise_t<object_t,except_t> conro_com_search( string_t search ) {
    return promise_t<object_t,except_t>([=]( res_t<object_t> res, rej_t<except_t> rej ){
        ...
        auto uri = regex::format( "curl https://www.conro.com/search-results/?searchterm=${0}&searchterm_submit=Go -Ls", search );
